...
function SafeDie({ faces, color, textColor = '#ffffff', result, rolling, symbol, onClick }) {
  ...
  return <button className={`safe-die safe-${shape} ${rolling ? 'is-rolling' : ''}`} style={{ '--die-color': color, '--face-text': textColor, '--rotate-x': `${rotation.x}deg`, '--rotate-y': `${rotation.y}deg`, '--rotate-z': `${rotation.z}deg` }} onClick={launch} aria-label="Lancer le dé"><span className="die-solid"><strong className="die-plane front">{front}</strong><strong className="die-plane back">{side}</strong><strong className="die-plane right">{side}</strong><strong className="die-plane left">{top}</strong><strong className="die-plane top">{top}</strong><strong className="die-plane bottom">{side}</strong></span></button>
}
...
function CustomDie({ die, onChange, onDelete }) {
  ...
  return <article className="roll-card" style={{ '--die-color': die.color }}>
    <div className="roll-card-top"><div className="die-actions"><ColorControl color={die.color} textColor={die.textColor || '#ffffff'} onColorChange={color => onChange({ color })} onTextColorChange={textColor => onChange({ textColor })} /><button className="icon-button" onClick={() => window.confirm(`Supprimer le dé « ${die.title} » ?`) && onDelete()} title="Supprimer"><Trash2 size={16} /></button></div></div>
